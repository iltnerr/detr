...
def _make_detr(backbone_name: str, dilation=False, num_classes=91, mask=False, pretrain_backbone=True):
    hidden_dim = 256
    backbone = Backbone(backbone_name, train_backbone=pretrain_backbone, return_interm_layers=mask, dilation=dilation)
...
def detr_resnet50(checkpoints_path: str, pretrained=False, num_classes=91):
    """
...
def detr_resnet101_panoptic(
    checkpoints_path: str, pretrained=False, num_classes=250, threshold=0.85, return_postprocessor=False):
    """
   ...
   threshold is the minimum confidence required for keeping segments in the prediction
    """
    pretrain_backbone = not pretrained
    model = _make_detr("resnet101", dilation=False, num_classes=num_classes, mask=True, pretrain_backbone=pretrain_backbone)
...
def detr_resnet101(checkpoints_path: str, pretrained=False, num_classes=91, return_postprocessor=False):
    """
    DETR-DC5 R101 with 6 encoder and 6 decoder layers.
    ...
    """
    pretrain_backbone = not pretrained
    model = _make_detr("resnet101", dilation=False, num_classes=num_classes, pretrain_backbone=pretrain_backbone)
    if pretrained:
        model.load_state_dict(torch.load(checkpoints_path)["model"])
    if return_postprocessor:
